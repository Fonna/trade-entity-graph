...
| M6 | FastAPI 接口 | 提供搜索、详情、图谱、审核写入、导出接口 | P0 | P0 API 可通过最小样例验证 |
| M7 | Streamlit MVP 页面 | 提供导入、搜索、图谱、详情、审核、导出原型页面 | P0 | 用户可完成演示脚本中的核心路径 |
...
当前通过标准：全量测试 17 passed，M8 专项测试 3 passed，ruff 0 errors。`.pytest_cache` 在当前桌面沙箱下可能出现写入权限 warning，不影响测试结果。
